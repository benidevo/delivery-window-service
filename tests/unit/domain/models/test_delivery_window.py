...
def test_should_process_windows_correctly() -> None:
    time_range1 = TimeRange(
        start_time=Time(hours=14, minutes=0), end_time=Time(hours=16, minutes=0)
    )
    time_range2 = TimeRange(
    ...
    complex_window = DeliveryWindow(day=DayOfWeek.MONDAY, windows=ranges)

    assert len(complex_window.windows) == 2
    assert complex_window.windows[0].start_time.hours == 8
    assert complex_window.windows[0].end_time.hours == 14
    assert complex_window.windows[1].start_time.hours == 16
    assert complex_window.windows[1].end_time.hours == 20
...
def test_should_intersect_two_delivery_windows() -> None:
    ...
    window3 = DeliveryWindow(day=DayOfWeek.TUESDAY)

    with pytest.raises(IncompatibleDaysError) as exc_info:
        window1.intersect_with(window3)

    assert "Cannot intersect different days" in str(exc_info.value)

    window4 = DeliveryWindow.closed(day=DayOfWeek.MONDAY)
    intersection = window1.intersect_with(window4)

    assert intersection.is_closed


def test_should_format_delivery_window() -> None:
